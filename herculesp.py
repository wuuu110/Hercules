...
class Herculesp:
    ...
    def init_prompt(self) -> None:
        ...
        self.system_predictor = file_to_string(f'{self.prompt_dir}/common/system_predictor.txt')
        self.user_predictor_crossover = file_to_string(f'{self.prompt_dir}/common/user_predictor_crossover.txt')
        self.user_predictor_mutation = file_to_string(f'{self.prompt_dir}/common/user_predictor_mutation.txt')
        ...
        self.user_direction_st_prompt = file_to_string(
            f'{self.prompt_dir}/common/user_direction_st.txt')
        self.user_direction_lt_prompt = file_to_string(
            f'{self.prompt_dir}/common/user_direction_lt.txt')  # long-term direction
        ...
        self.print_short_term_direction_prompt = True  # Print short-term direction prompt for the first iteration
        self.print_long_term_direction_prompt = True  # Print long-term direction prompt for the first iteration
    ...
    def init_population(self) -> None:
        ...
        responses = self.generator_llm.multi_chat_completion([messages], self.cfg.init_pop_size,
                                                             temperature=self.generator_llm.temperature + 0.3)  # Increase the temperature for diverse initial population
    ...
    def update_iter(self) -> None:
        ...
        for response_id in range(len(population)):
            logging.info(
                f"Iteration {self.iteration}, response_id {response_id}: Objective value: {population[response_id]['obj']}")

        objs_with_indices = [(i, individual["obj"]) for i, individual in enumerate(population) if
                             individual["exec_success"] == True]
        indices, objs = zip(*objs_with_indices)

        best_obj, best_sample_idx = min(objs), indices[np.argmin(objs)]
        worst_obj, worst_sample_idx = max(objs), indices[np.argmax(objs)]
        ...
        if self.mediocrity is None or worst_obj > self.mediocrity["obj"]:
            self.mediocrity = population[worst_sample_idx]
            logging.info(f"Iteration {self.iteration}: Mediocrity: {self.mediocrity['obj']}")
    ...
    def predict(self, population, crossover_flag, alpha):
        """
        PPP:  Hercules-P is based on the ReEvo implementation, crossover and mutation generate inconsistent numbers of heuristics.
        Therefore, we use a flag to distinguish between the two operators.
        crossover derives ten heuristics
        mutation derives five heuristics
        """


        predictor_example = self.predictor_population
        for response_id in range(len(predictor_example)):
            if predictor_example[response_id]["obj"] == float("inf"):
                raise RuntimeError("Exisiting code", response_id, "with inf performance score")

        boundary = (predictor_example[1]["obj"] - predictor_example[0]["obj"])
        alpha_pop = int(alpha * len(population))

        #crossover: need to predict ten heuristics
        if crossover_flag == True:
            system = self.system_predictor
            user = self.user_predictor_crossover.format(
                func_name=self.func_name,
                alg=self.alg,
                pro=self.pro,
                func_desc=self.func_desc,
                code_0=predictor_example[0]["code"],
                score_0=predictor_example[0]["obj"],
                code_1=predictor_example[1]["code"],
                score_1=predictor_example[1]["obj"],
                code_2=predictor_example[2]["code"],
                score_2=predictor_example[2]["obj"],
                code_3=predictor_example[3]["code"],
                score_3=predictor_example[3]["obj"],
                code_4=predictor_example[4]["code"],
                score_4=predictor_example[4]["obj"],
                code_5=predictor_example[5]["code"],
                score_5=predictor_example[5]["obj"],
                code_6=predictor_example[6]["code"],
                score_6=predictor_example[6]["obj"],
                code_7=predictor_example[7]["code"],
                score_7=predictor_example[7]["obj"],
                code_8=predictor_example[8]["code"],
                score_8=predictor_example[8]["obj"],
                code_9=predictor_example[9]["code"],
                score_9=predictor_example[9]["obj"],
                code_10=population[0]["code"],
                code_11=population[1]["code"],
                code_12=population[2]["code"],
                code_13=population[3]["code"],
                code_14=population[4]["code"],
                code_15=population[5]["code"],
                code_16=population[6]["code"],
                code_17=population[7]["code"],
                code_18=population[8]["code"],
                code_19=population[9]["code"],
            )
        else:  # crossover: predict five heuristics
            system = self.system_predictor
            user = self.user_predictor_mutation.format(
                func_name=self.func_name,
                alg=self.alg,
                pro=self.pro,
                func_desc=self.func_desc,
        ...
        response = self.direction_llm.multi_chat_completion([message])

        logging.info("-----------predict performance-------")
        pattern = r'code_(\d+): (\d+\.\d+), (0\.\d+)'
        input_string = ' '.join(response)
        matches = re.findall(pattern, input_string)


        if len(matches) != len(population):
            logging.warning(
                "There was an error in the prediction process. Please check the prediction output of LLM.")
            logging.warning("LLM response:"+str(response))
            for ind in range(len(population)):
                population[ind]["obj"] = float("inf")
                population[ind]["exec_success"] = False
        else:
            filtered_matches = [match for match in matches if 1-2*self.delta <= float(match[2]) < 1 - self.delta]
            filtered_matches_sorted = sorted(filtered_matches, key=lambda x: float(x[2]), reverse=True)

            top_m_matches = filtered_matches_sorted[:alpha_pop]

            for ind in range(len(population)):
                try: # ConS mechanism
                    match = matches[ind]
                    if ind == (int(match[0]) - len(predictor_example)) and float(match[1]) > predictor_example[0]["obj"] and float(match[1]) < predictor_example[1]["obj"]:

                        if float(match[2]) >= 1-self.delta:
                            population[ind]["obj"] = float(match[1])
                            population[ind]["exec_success"] = True
                        elif match in top_m_matches:
                            population[ind]["obj"] = float(match[1])
                            population[ind]["exec_success"] = True
                        elif float(match[2]) >1- 3*self.delta and float(match[2]) < 1-2*self.delta and float(match[1]) > predictor_example[0]["obj"] + 3*boundary*self.delta:
                            population[ind]["obj"] = float(match[1])
                            population[ind]["exec_success"] = True
                        else:
                            population[ind]["obj"] = float("inf")
                    ...
                    logging.warning(f"Cannot assign a objective value to the {ind} th individual.")
                    population[ind]["obj"] = float("inf")
                    population[ind]["exec_success"] = False

        """for ind in range(len(population)):
            logging.info(f'predicted performance score {population[ind]["obj"]}')"""

        return population

    def construct_predictor(self, selected_pop):
        """
            EXEMPLAR mechanism : select heuristic examples for PPP
        """
        selected_population = [item for item in selected_pop if item["obj"] != float("inf")]
        current_population = [self.elitist, self.mediocrity]


        # select the best and worst heuristic as prediction boundaries
        if self.elitist in selected_population:
            selected_population.remove(self.elitist)
        if self.mediocrity in selected_population:
            selected_population.remove(self.mediocrity)

        used_obj = {self.elitist["obj"], self.mediocrity["obj"]}
        for _ in range(self.predictor_number - 2):
            available_individuals = [ind for ind in selected_population if ind["obj"] not in used_obj]
            if not available_individuals:
                break
            available_individuals = sorted(available_individuals, key=lambda x: x["obj"])
            individual = available_individuals[0]
            used_obj.add(individual["obj"])
            current_population.extend([individual])

        # if the number of heuristics is insufficient.
        if len(current_population) < self.predictor_number:
            additional_ind_needed = self.predictor_number - len(current_population)
            remaining_individuals = [ind for ind in selected_population if ind not in current_population]
            current_population.extend(np.random.choice(remaining_individuals, size=additional_ind_needed,
                                                       replace=False))

        """for individual in range(len(current_population)):
            logging.info(f'----predictor example{individual}---obj'+str(current_population[individual]["obj"]))
            logging.info(str(current_population[individual]["code"]))"""

        self.predictor_population = current_population
        # update current predictor population
    ...
    def evolve(self):
        ...
        while self.function_evals < self.cfg.max_fe:
            ...
            #Select heuristic examples for PPP
            self.construct_predictor(selected_population)
            ...
            if self.function_evals <= self.lamda * self.cfg.max_fe:
                ...
                core_abstraction = ""
            # Short-term direction
            short_term_direction_tuple = self.short_term_direction(selected_population,
                                                                         core_abstraction)  # (response_lst, worse_code_lst, better_code_lst)
            # Crossover
            crossed_population = self.crossover(short_term_direction_tuple)

            # Flag is used to distinguish between crossover and mutation operator
            flag = True
            #PPP
            pop = self.predict(crossed_population, flag, self.alpha)
            evaluate_pop = [ind for ind in pop if ind["obj"] == float('inf')]
            predict_pop = [ind for ind in pop if ind["obj"] != float('inf')]

            ## Evaluate part of newly derived heuristics
            self.population = self.evaluate_population(evaluate_pop)
            self.population.extend(predict_pop)
            self.function_evals = self.function_evals +len(predict_pop)

            # Update
            self.update_iter()
            # Long-term direction
            self.long_term_direction([response for response in short_term_direction_tuple[0]],
                                          core_abstraction)
            # Mutate
            ...
            flag = False
            pop = self.predict(mutated_population, flag, self.alpha)
            evaluate_pop = [ind for ind in pop if ind["obj"] == float('inf')]
            predict_pop = [ind for ind in pop if ind["obj"] != float('inf')]

            # Evaluate
            self.population.extend(self.evaluate_population(evaluate_pop))
            self.population.extend(predict_pop)
            self.function_evals = self.function_evals + len(predict_pop)
            # Update
            self.update_iter()
